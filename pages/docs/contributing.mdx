...
Pull requests are welcome for any part of the repository - tests, the package
...
-   PRs must be formatted with prettier (prettier config is included in the
    repository).
...
-   Short PRs may be self-explanatory, but larger PRs should offer some
    explanation of the changes made.
-   If your PR adds new features or changes existing functionality, you don't
    have to update the docs yourself, but you should provide enough information
    that it is easy for others to do so.

Other than that, have at it! If your PR changes/adds functionality, you don't
have to update the docs yourself, but you should provide enough information that
it is easy for others to do so.
...
-   Be written using <ExLink href={moo}>Moo</ExLink>. _PRs that add new
    dependencies to tokenize code will not be approved_.
...
some of the assumptions that were made for JavaScript/TypeScript.
...
-   Include a rationale for the change. Is it faster? More concise? Did the
    original not work as expected? A brief explanation goes a long way!

_Changes that would potentially break the most current release of lexour are
still highly encouraged, but their approval may take longer than non-breaking
changes_.
